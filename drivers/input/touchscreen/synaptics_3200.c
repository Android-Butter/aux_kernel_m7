...
static void home2menu_pressmenu(struct work_struct * home2menu_pressmenu_work) {
...
static DECLARE_WORK(home2menu_pressmenu_work, home2menu_pressmenu); 

void home2menu_menutrigger(void) {
                schedule_work(&home2menu_pressmenu_work);
        return;
} 
...
static void home2wake_longtap_count(struct work_struct * home2wake_longtap_count_work)
{
	int time_count = 0;
...
static DECLARE_WORK(home2wake_longtap_count_work, home2wake_longtap_count);

void home2wake_longtap_count_trigger(void)
{
	break_longtap_count = 0;
	if (allow_longtap_count) {
		allow_longtap_count = 0;
		printk("[L2W] starting longtap count work\n");
		schedule_work(&home2wake_longtap_count_work);
	} else {
...
static ssize_t synaptics_home2menu_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t count = 0;
	count += sprintf(buf, "%d\n", l2m_switch);
...
static ssize_t synaptics_home2menu_dump(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] >= '0' && buf[0] <= '1' && buf[1] == '\n')
...
static DEVICE_ATTR(home2menu, (S_IWUSR|S_IRUGO),
	synaptics_home2menu_show, synaptics_home2menu_dump); 

static ssize_t synaptics_home2wake_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t count = 0;
	count += sprintf(buf, "%d\n", l2w_switch);
...
static ssize_t synaptics_home2wake_dump(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] >= '0' && buf[0] <= '1' && buf[1] == '\n')
...
static int synaptics_touch_sysfs_init(void)
{
	...
	if (get_address_base(gl_ts, 0x54, FUNCTION))
		...
		sysfs_create_file(android_touch_kobj, &dev_attr_home2menu.attr) ||
		sysfs_create_file(android_touch_kobj, &dev_attr_home2wake.attr)
...
static void synaptics_touch_sysfs_remove(void)
{
	...
	sysfs_remove_file(android_touch_kobj, &dev_attr_home2menu.attr);
...
static void home2menu_func(void) {
	...
	if (l2m_switch == 1 && scr_suspended == false && ((l2m_time[0]-l2m_time[1]) < L2M_TIMEOUT)) {
		...
		home2menu_menutrigger();
	} 

        return;
}

static void home2wake_func(void) {

	if (l2w_switch == 1) {
		home2wake_longtap_count_trigger();
	}	
